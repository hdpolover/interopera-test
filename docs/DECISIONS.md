...
**External-data boundary:** because bounds are now graph data rather than validated config, `compute_figure` treats them as untrusted at the boundary — it resolves the figure's bounds once and verifies the comparator's required keys (`_REQUIRED_BOUNDS`) are present. A missing or partial `Threshold` produces an `ERROR` figure (never a `KeyError` crash or a silently wrong status), the same discipline applied to unresolvable citations and `PENDING_REVIEW` nodes.
